...
def _style_sheet(ws: Any, df: pd.DataFrame, score_col_idx: int) -> None:
    ...
    # Auto-width columns with per-column caps so long text stays readable
    # without making the entire sheet too wide.
    width_caps = {
        "Rank": 8,
        "Candidate Name": 20,
        "Email": 24,
        ...
        "Has Projects": 12,
        "Job Hopping": 12,
        "Must-have Missing": 24,
        "Nice-to-have Missing": 24,
        "Tags": 22,
        "Red Flags": 22,
        "Skills Found": 28,
        "Skills Missing": 28,
    ...
    for col_idx, col_name in enumerate(df.columns, start=1):
        ...
        cap = width_caps.get(str(col_name), 26)
...
def _find_windows_font(font_name: str) -> str | None:
    """Return the full path to a Windows font file if it exists."""
    font_path = os.path.join(os.environ.get("WINDIR", r"C:\Windows"), "Fonts", font_name)
    return font_path if os.path.exists(font_path) else None


def _configure_pdf_fonts(pdf: FPDF) -> bool:
    """Try to register a Unicode-capable Arial font family for PDF output."""
    font_files = {
        "": _find_windows_font("arial.ttf"),
        "B": _find_windows_font("arialbd.ttf"),
        "I": _find_windows_font("ariali.ttf"),
        "BI": _find_windows_font("arialbi.ttf"),
    }
    if not font_files[""]:
        return False

    pdf.add_font("ArialUnicode", "", font_files[""])
    if font_files["B"]:
        pdf.add_font("ArialUnicode", "B", font_files["B"])
    if font_files["I"]:
        pdf.add_font("ArialUnicode", "I", font_files["I"])
    if font_files["BI"]:
        pdf.add_font("ArialUnicode", "BI", font_files["BI"])
    return True


def _pdf_text(value: object, max_len: int = 200, unicode_font: bool = False) -> str:
    """Prepare text for PDF output with optional Unicode preservation."""
    text = str(value) if value is not None else ""
    text = _ILLEGAL_CHARS_RE.sub("", text)
    text = text[:max_len]
    if unicode_font:
        return text
    return _safe_str(text, max_len)
...
def to_pdf_bytes(results: list[CVResult], jd_snippet: str = "") -> bytes:
    ...
    unicode_font = _configure_pdf_fonts(pdf)
    ...
    if jd_snippet:
        ...
        pdf.multi_cell(0, 6, f"JD: {_pdf_text(jd_snippet, 160, unicode_font)}")
        pdf.ln(4)
    ...
    for rank, r in enumerate(results, start=1):
        ...
        score_pct = round(r.score * 100, 1)
        name_display = _pdf_text(r.candidate_name or r.filename, 60, unicode_font)
        ...
        pdf.cell(0, 6, _pdf_text(r.email or "-", unicode_font=unicode_font), ln=True)
        pdf.set_font(font_family, "B", 9)
        pdf.cell(35, 6, "Phone:", border=0)
        pdf.set_font(font_family, "", 9)
        pdf.cell(0, 6, _pdf_text(r.phone or "-", unicode_font=unicode_font), ln=True)
        pdf.set_font(font_family, "B", 9)
        pdf.cell(35, 6, "File:", border=0)
        pdf.set_font(font_family, "", 9)
        pdf.cell(0, 6, _pdf_text(r.filename, 80, unicode_font), ln=True)
        pdf.ln(3)
        ...
        if r.skills_found:
            ...
            pdf.set_font(font_family, "", 9)
            pdf.multi_cell(0, 6, _pdf_text(", ".join(r.skills_found), 300, unicode_font))
            pdf.ln(2)
        ...
        if r.skills_missing:
            ...
            pdf.set_font(font_family, "", 9)
            pdf.multi_cell(0, 6, _pdf_text(", ".join(r.skills_missing), 300, unicode_font))
            pdf.ln(2)
        ...
        if r.must_have_missing:
            ...
            pdf.set_font(font_family, "", 9)
            pdf.multi_cell(0, 6, _pdf_text(", ".join(r.must_have_missing), 200, unicode_font))
            pdf.ln(2)
        ...
        if r.tags:
            ...
            pdf.set_font(font_family, "", 9)
            pdf.multi_cell(0, 6, _pdf_text(" | ".join(r.tags), 200, unicode_font))
            pdf.ln(2)
        ...
        if r.red_flags:
            ...
            for flag in r.red_flags:
                # multi_cell() can leave X at the right edge; reset to left margin
                # before each bullet to avoid zero-width wrapping on the next item.
                pdf.set_x(pdf.l_margin)
                pdf.multi_cell(0, 6, f"  - {_pdf_text(flag, 120, unicode_font)}")
            pdf.ln(2)
        ...
        pdf.set_font(font_family, "", 8)
        pdf.multi_cell(0, 5, _pdf_text(r.summary, 500, unicode_font))
