...
def _check_groq_status(api_key: str, model: str) -> tuple[bool, str]:
    ...
    try:
        ...
        return True, f"Model: {model or 'llama-3.1-8b-instant'} · Phản hồi thử: \"{reply}\""
...
def _norm_skill_text(value: str) -> str:
    """Normalise a skill string for tolerant matching."""
    return re.sub(r"[^a-z0-9+#]+", " ", (value or "").lower()).strip()


@st.cache_resource(show_spinner=False)
def _build_skill_alias_index() -> tuple[dict[str, str], list[str], list[str], np.ndarray]:
    """Build a cached alias index + embeddings for semantic skill mapping."""
    alias_to_canonical: dict[str, str] = {}
    alias_norm_list: list[str] = []
    alias_canonical_list: list[str] = []

    for canonical, aliases in SKILL_KEYWORDS.items():
        for alias in [canonical, *aliases]:
            norm_alias = _norm_skill_text(alias)
            if not norm_alias:
                continue
            if norm_alias not in alias_to_canonical:
                alias_to_canonical[norm_alias] = canonical
                alias_norm_list.append(norm_alias)
                alias_canonical_list.append(canonical)

    alias_embeddings = encode(alias_norm_list)
    return alias_to_canonical, alias_norm_list, alias_canonical_list, alias_embeddings


def _map_llm_skills_to_catalog(skills: list[str]) -> tuple[list[str], list[str], dict[str, float]]:
    """
    Map LLM-extracted skills to canonical skills from ``SKILL_KEYWORDS``
    using exact, fuzzy substring, and semantic-embedding matching.

    Returns
    -------
    tuple[list[str], list[str], dict[str, float]]
        (mapped_canonical_skills, unmapped_raw_skills, confidence_by_skill)
    """
    alias_to_canonical, alias_norms, alias_canonicals, alias_embeddings = _build_skill_alias_index()
    ...
    _SUBSTRING_MATCH_CONFIDENCE = 0.90
    ...
    for raw in skills:
        ...
        key = _norm_skill_text(raw_text)
        if not key:
            continue

        canonical = alias_to_canonical.get(key)
        conf = 1.0 if canonical else 0.0

        if canonical is None:
            # Fallback: substring match against aliases (helps with labels like
            # "Python programming" or "AWS cloud").
            for alias_key, alias_canonical in alias_to_canonical.items():
                if not alias_key:
                    continue
                if alias_key in key or key in alias_key:
                    canonical = alias_canonical
                    conf = _SUBSTRING_MATCH_CONFIDENCE
                    break

        if canonical is None:
            raw_emb = encode([key])[0]
            sims = alias_embeddings @ raw_emb
            best_idx = int(np.argmax(sims))
            best_sim = float(sims[best_idx])
            if best_sim >= _SEMANTIC_MATCH_THRESHOLD:
                canonical = alias_canonicals[best_idx]
                conf = best_sim

        if canonical:
            mapped.append(canonical)
            confidence_by_skill[canonical] = max(confidence_by_skill.get(canonical, 0.0), conf)
        else:
            unmapped.append(raw_text)

    return list(dict.fromkeys(mapped)), list(dict.fromkeys(unmapped)), confidence_by_skill


def _select_freeform_llm_skills(skills: list[str]) -> list[str]:
    """Keep only short, skill-like phrases safe for literal CV matching."""
    selected: list[str] = []
    for raw in skills:
        text = (raw or "").strip()
        norm = _norm_skill_text(text)
        if not norm:
            continue
        word_count = len(norm.split())
        # Avoid long sentence-like items from LLM output.
...
def _match_freeform_skills_in_text(text: str, skills: list[str]) -> list[str]:
    """Find free-form skill phrases by strict normalised phrase matching."""
    norm_text = f" {_norm_skill_text(text)} "
    found: list[str] = []
    for skill in skills:
        token = _norm_skill_text(skill)
        if token and f" {token} " in norm_text:
            found.append(skill)
    return list(dict.fromkeys(found))


def _jd_preview_cache_key(jd_text: str, llm_cfg: LLMConfig) -> str:
    """Build a stable key to reuse JD AI preview across reruns."""
    return f"{llm_cfg.provider}|{llm_cfg.model}|{_text_fingerprint(jd_text)}"


def _text_fingerprint(text: str) -> str:
    """Return a stable fingerprint for cache keys."""
    return hashlib.sha256((text or "").encode("utf-8", errors="ignore")).hexdigest()


def _bytes_fingerprint(data: bytes) -> str:
    """Return a stable fingerprint for binary cache keys."""
    return hashlib.sha256(data).hexdigest()


def _jd_summary_cache_key(jd_summary: JDSummary) -> tuple:
    """Return a hashable summary key for memoizing analysis results."""
    return (
        jd_summary.position_title,
        jd_summary.level,
        jd_summary.min_experience,
        tuple(jd_summary.must_have),
        tuple(jd_summary.nice_to_have),
        tuple(jd_summary.all_skills),
    )


def _extract_uploaded_file_text(uploaded_file) -> str:
    """Extract text from an uploaded file with session-level memoization."""
    cache = st.session_state.setdefault("_cv_text_cache", {})
    file_bytes = uploaded_file.getvalue()
    cache_key = f"{uploaded_file.name.lower()}|{_bytes_fingerprint(file_bytes)}"
...
def _analyze_cv_cached(
    ...
    api_key: str = "" # Bắt buộc có api_key
) -> CVResult:
    """Memoize CV analysis (Phiên bản AI)"""
    cache = st.session_state.setdefault("_cv_analysis_cache", {})
    # Tạo key cache đơn giản hơn
    cache_key = f"{filename}_{hash(cv_text)}_{hash(jd_text)}"
        ...
        with st.container(border=True):
            ...
            if llm_enabled:
                ...
                if current_provider == "groq" and _ENV_GROQ_KEY:
                    st.info("🔑 Đang dùng API key từ file .env")
                    # Update session state if environment key changed
                    if st.session_state.get("llm_api_key") != _ENV_GROQ_KEY:
                        st.session_state["llm_api_key"] = _ENV_GROQ_KEY
                    ...
                    llm_model = st.text_input(
                        "Model (để trống = mặc định)",
                        placeholder="llama-3.1-8b-instant / gpt-4o-mini / gemini-1.5-flash",
                        key="llm_model",
                    )
                    if llm_api_key:
                    ...
                    if _api_key_to_test:
                        ...
                        if _groq_ok:
                            ...
                            st.error(
                                ...
                                "- Model không tồn tại → thử `llama-3.1-8b-instant`\n"
...
def _render_insight_dashboard(
    ...
    col_chart1, col_chart2 = st.columns([1, 1.25])

    # ---- donut distribution ----
    with col_chart1:
        donut_pairs = [
            ("🟢 Phù hợp cao", high, "#2ecc71"),
            ("🟡 Trung bình", med, "#f39c12"),
            ("🔴 Chưa phù hợp", low, "#e74c3c"),
        ]
        donut_pairs = [p for p in donut_pairs if p[1] > 0]

        fig_donut = go.Figure(data=[
            go.Pie(
                labels=[p[0] for p in donut_pairs] or ["Chưa có dữ liệu"],
                values=[p[1] for p in donut_pairs] or [1],
                hole=0.55,
                marker_colors=[p[2] for p in donut_pairs] or ["#95a5a6"],
    ...
    with col_chart2:
        if jd_skills:
            ...
            for skill in jd_skills:
                ...
                skill_rows.append((skill, pct))

            # Show high-coverage skills first for easier scanning.
            skill_rows.sort(key=lambda x: x[1], reverse=True)
            skill_labels = [s for s, _ in skill_rows]
            skill_pcts = [p for _, p in skill_rows]

            chart_height = int(min(1000, max(420, 120 + 30 * len(skill_labels))))
            left_margin = int(min(460, max(180, 8 * max((len(s) for s in skill_labels), default=10))))
            ...
            fig_skills.update_layout(
                ...
                yaxis=dict(autorange="reversed", tickfont=dict(size=14), automargin=True),
                height=chart_height,
                margin=dict(l=left_margin, r=30, t=40, b=20),
            )
...
def _render_candidate_radar(
    ...
    st.plotly_chart(fig, width="stretch", key=f"cand_radar_{key_suffix}")
            ...
            with tab_ai:
                if cached_llm:
                    ...
                    if cached_llm.potential_level:
                        ai_c4.metric("Tiềm năng", cached_llm.potential_level)
...
def render_analysis_tab():
    ...
    with col_cv:
        ...
        uploaded_files = st.file_uploader(
            label="Upload một hoặc nhiều file CV",
            type=["pdf", "docx"],
    ...
    # ---- PHÂN TÍCH JD (SẠCH SẼ, BỎ NHÃN TÍM) ----
    jd_summary = JDSummary()
    current_api_key = st.session_state.get("llm_api_key", "")

    if jd_text and jd_text.strip():
        if not current_api_key:
            st.warning("⚠️ Vui lòng cấu hình Groq API Key ở Sidebar để phân tích.")
        else:
            with st.spinner("AI đang đọc hiểu yêu cầu công việc..."):
                jd_summary = parse_jd(jd_text, current_api_key)
            st.divider()
            _render_jd_summary_card(jd_summary)
            _render_pre_filter(jd_summary)
            # ĐÃ XÓA TOÀN BỘ CODE TẠO NHÃN TÍM (AI PREVIEW CŨ) Ở ĐÂY
    ...
    if run:
        with st.spinner("🔍 AI đang chấm điểm CV…"):
            ...
            for i, uploaded_file in enumerate(uploaded_files):
                ...
                progress.progress((i + 1) / len(uploaded_files), text=f"Đã xử lý: {uploaded_file.name}")

            progress.empty()

            if errors:
                st.warning(f"⚠️ Không thể đọc chữ từ file: {', '.join(errors)}")
...
def _render_shortlist_tab(
    ...
    st.dataframe(sl_df, width="stretch")
